<?php
include '../../../config.php';
include PATH_INCLUDE . 'TemplatePages.php';
include PATH_INCLUDE . 'ActividadH5P.php';
include PATH_INCLUDE . 'Videos.php';
include PATH_INCLUDE . 'ImagenPie.php';

$urlPath = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
$menuAsignaturaPath = getMenuAsignaturaPath($urlPath);
ob_start();
?>
<section>
  <h2>Ma nationalité</h2>
  <h3>Mi nacionalidad</h3>
  <p>En esta lección, aprenderás a decir tu nacionalidad.</p>
  <?php ob_start(); ?>
  <p><strong>Instrucciones:</strong></p>
  <ol class="ol-number md:ml-32 mb-8">
    <li>Regardez la vidéo suivante du début à la seconde 00:24 / Ve el siguiente vidéo desde el inicio hasta el segundo 00:24.</li>
    <li>Répondez aux questions suivantes / Responde las siguientes preguntas:</li>
  </ol>
  <div class="max-w-xl mx-auto bg-cyan-700/30 p-4 my-6">
    <?php
    renderVideoIframe('NqzPWpKbnhc', 'Les adjectifs de nationalité en français. How to talk about nationality in French?');
    ?>
  </div>
  <?php
  $ActividadContent = ob_get_clean();
  renderActividadH5P('u2-act-3', "Ma nationalité", $ActividadContent);
  ?>
  <p class="mt-10 font-bold">Conceptualización:</p>
  <p>Después de haber visto el video, deducimos que la pregunta "D'où venez-vous?" se plantea para saber el país de origen de la persona. No obstante, para saber cuál es la nacionalidad, la pregunta que emplearemos es: "Quelle est ta nationalité?"</p>
  <p>Y la oración que se emplea para expresar nuestra nacionalidad es "Je suis". Y cuando es el caso de los mexicanos, que son varias personas juntas las que contestan, es "nous sommes". Es decir, utilizamos el verbo "être" conjugado en presente.</p>
  <p>La sintaxis completa de la oración en singular queda así:</p>
  <div class="grid grid-cols-8 gap-2">
    <div class="col-start-2 text-xl text-center">Sujet</div>
    <div class="text-5xl text-center">+</div>
    <div class="text-xl text-center">verbe (être)</div>
    <div class="text-5xl text-center">+</div>
    <div class="text-xl text-center col-span-2">adjectif qualificatif (la nationalité)</div>
  </div>
  <div class="grid grid-cols-8 gap-2">
    <div class="col-start-2 font-bold text-center text-rose-700">Je</div>
    <div class="font-bold text-center text-fuchsia-900"> </div>
    <div class="font-bold text-center text-blue-700">suis</div>
    <div class="font-bold text-center text-fuchsia-900"> </div>
    <div class="font-bold text-center text-fuchsia-900 col-span-2">australien</div>
  </div>
  <p class="mt-4">Recuerda que en francés realizamos "l'accord de genre". Es decir, que se deben de adecuar los sustantivos o, en este caso, los adjetivos con el género del sujeto, o sea, de la persona que habla o a la que nos referimos. Ve el siguiente video y observa las reglas de formación del adjetivo de nacionalidad: </p>
  <div class="w-2/3 mx-auto bg-fuchsia-700/30 p-4 my-6">
    <?php
    renderVideoIframe('MU6X8_bnSx8', 'Dire sa nationalité en français');
    ?>
  </div>
  <p><strong>Instrucciones:</strong></p>
  <ol class="ol-number md:ml-32 mb-8">
    <li>En fonction de ce que vous avez observé, réalisez l'activité suivante / A partir de lo que observaste, haz la siguiente actividad.</li>
    <li>Complète avec le mot qui convient / Completa con la palabra correcta.</li>
  </ol>
  <?php ob_start(); ?>
  <p>Arrastra las palabras a las cajas correspondientes.</p>
  <?php
  $ActividadContent = ob_get_clean();
  renderActividadH5P('u2-act-5', "Nationalité en français", $ActividadContent, true);
  ?>
  <div class="w-full grid grid-cols-3 gap-4 mt-8 border-y-2 border-dashed border-purple-300 bg-purple-300/10">
    <div class="col-span-2 p-2">
      <div class="text-purple-800 font-bold text-2xl text-center mt-6">¡Atención!</div>
      <div class="text-purple-800 mt-6 pl-6">En formularios de documentos donde te solicitan rellenar con tu nacionalidad, es decir, la "nationalité", ésta siempre se escribe en femenino. Como en el ejemplo de este pasaporte:</div>
    </div>
    <div class="pr-6 pt-3">
      <div class="shadow-xl">
        <?php
        renderImage('f1u2t2p1img1.webp');
        ?>
      </div>
    </div>
  </div>
  <div class="w-full grid grid-cols-5 gap-4 mt-4 border-y-2 border-dashed border-cyan-300 bg-cyan-300/10">
    <div class="pl-6 pt-6">
      <div class="shadow-xl">
        <?php
        renderImage('f1u2t2p1img2.webp');
        ?>
      </div>
    </div>
    <div class="col-span-4 pl-2 pr-8">
      <div class="text-cyan-800 font-bold text-xl mt-2">DATO IMPORTANTE:</div>
      <div class="text-cyan-800">En el pasaporte actual francés, en la parte de <em>nationalité</em>, ya solo se ponen las primeras letras de la nacionalidad. </div>
    </div>
  </div>
</section>


<?php
$content = ob_get_clean();
renderTemplatePage($menuAsignaturaPath, $content);
?>
